...
class ViewUtils:
    ...
    def generic_detail_paginated_response( #! Tested FAIL
                view_instance : generics.GenericAPIView,
                request,
        ...
        paginator = view_instance.pagination_class()
        queryset = view_instance.filter_queryset(items)
        serializer = DynamicModelSerializer(paginator.paginate_queryset(queryset, request), model_class=model_class, many=True)
...
class DynamicModelSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        # Get the model class dynamically from the instance if not provided
        if 'model_class' in kwargs:
            self.Meta.model = kwargs.pop('model_class')
        else:
            instance = kwargs.get('instance')
            if instance:
                self.Meta.model = instance._meta.model

        # Ensure model is not None
        if not self.Meta.model:
            raise ValueError("Model class must be provided to DynamicModelSerializer.")

        # Dynamically set fields based on the model's fields
        self.Meta.fields = [field.name for field in self.Meta.model._meta.fields]

        # Add reverse relationship fields
        self.reverse_relationships = [
            related.get_accessor_name() for related in self.Meta.model._meta.related_objects
        ]
    ...
    def to_representation(self, instance):
        """
        Convert `instance` to a dictionary of primitive datatypes,
        including reverse relationship fields.
        """
        ret = super().to_representation(instance)

        # Include reverse relationship fields
        for related_field in self.reverse_relationships:
            related_manager = getattr(instance, related_field)
            related_queryset = related_manager.all()
            ret[related_field] = [
                DynamicModelSerializer(rel_instance, context=self.context).data
                for rel_instance in related_queryset
            ]
